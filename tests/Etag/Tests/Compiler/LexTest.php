...
class LexTest extends EtagTestCase
{
    ...
    public function lexProvider()
    {
        return [
            ...
            [' {{$foo}}',
                [['TEXT', ' ', 1 , 1], ['TAG_START', '{{', 1, 2], ['VARIABLE', '$foo', 1, 4],['TAG_END','}}',1,8]],
            ],
            [' {{ $foo       }}',
            	[['TEXT',' ',1 , 1], ['TAG_START','{{',1,2], ['VARIABLE', '$foo',1,5], ['TAG_END','}}', 1, 16]],
    		],
            [" {{ \$foo\n}}",
                [['TEXT',' ',1 , 1], ['TAG_START','{{',1,2], ['VARIABLE','$foo',1,5], ['TAG_END','}}',2,1]],
    		],
            [" {{ \$foo\r\n}}",
    		    [['TEXT',' ',1 , 1], ['TAG_START','{{',1,2], ['VARIABLE', '$foo',1,5], ['TAG_END', '}}',2,1]],
            ],
            [" {{ \n\$foo\n}}", 
                [['TEXT',' ',1 , 1], ['TAG_START','{{',1,2], ['VARIABLE', '$foo', 2, 1], ['TAG_END', '}}', 3, 1]],
    	    ],
        ];
...
//                ->append(new Token(Token::TYPE_TAG_END,'}}',1))->build()],
    ...
    public function testLex($tplCode, $expected)
    {
        ...
        while (!$tokenStream->isEOF()) {
            ...
            $results[] = [substr($current->getTypename(), 5), $current->value, $current->line, $current->col];
        }
